...
$.fn.dataTable.ext.renderer.pageButton.bootstrap = function (settings, host, idx, buttons, page, pages) {
    ...
    var attach = function (container, buttons) {
        ...
        // START 2019-10-30添加，用于输入分页页码
        $(".gotoPage").remove();
        var pageHtml = "<span class='gotoPage' style='margin-left: 10px;color: #777;'>";
        	pageHtml+= "<span>到第</span>";
        	pageHtml+= "<input type='text' id='"+settings.sTableId+"_gotoPage' min='1' max='"+pages+"' class='form-control input-inline ' style='height:30px;width:50px !important;text-align: center;padding:0px 0px;' data-prev='"+(page+1)+"' value='"+(page+1)+"' title='输入页码后回车，便可跳转'>";
        	pageHtml+= "<span>页</span>";
        	pageHtml+= "</span>";
        $(pageHtml).appendTo(container);
        //绑定按钮事件
        $("#"+settings.sTableId+"_gotoPage").on('change',function(){
        	var gotoPage_val = $(this).val();
        	var max_val =$(this).attr("max");
        	var min_val =$(this).attr("1");
        	if (gotoPage_val == null || gotoPage_val === '' || gotoPage_val.match(/[^0-9]/) || parseInt(gotoPage_val)>parseInt(max_val) || parseInt(gotoPage_val)<parseInt(min_val)) {
        		// 没有输入或者输入了非数字，清除非数字
        		$(this).val('');
        		return;
        	}else if(parseInt(gotoPage_val) == settings.iDraw){
        		return;
        	}
        	//跳转页码
        	 api.page(parseInt(gotoPage_val)-1).draw(false);
        });
        //END
    };
